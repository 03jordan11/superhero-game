extends SceneTree
const FOLDER := "res://assets/buildings/bank2/"
...
func _initialize() -> void:
	...
	var hall := Node3D.new()
	hall.name = "Bank2"
	hall.set_script(load(FOLDER + "bank2.gd"))
	owned(hall, (load(FOLDER + "bank2.glb") as PackedScene).instantiate(), "Model")
	var body := StaticBody3D.new()
	owned(hall, body, "ExteriorCollision")
	var manifest: Dictionary = JSON.parse_string(FileAccess.get_file_as_string(FOLDER + "bank2_manifest.json"))
	...
	for entry: Dictionary in manifest.props:
		...
		prop.owner = hall
	save(hall, FOLDER + "bank2.tscn")
	hall.free()
	print("BANK2_SCENE_READY")
	quit()
