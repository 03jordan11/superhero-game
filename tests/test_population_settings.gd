...
func _run() -> void:
	...
	for cycle in range(3):
		for density in range(3):
			for distance in range(3):
				...
				assert(lod.population_target <= roundi(160*factor) and lod.far_population_target <= roundi(200*factor))
	...
	assert(manager.population_target == 20 and lod.population_target == 160 and lod.far_population_target == 200)
	assert(lod.max_boxes == 240 and lod.far_max_proxies == 320 and capsules.max_capsules == 120)
	...
	assert(lod.population_target == 29 and lod.far_population_target == 36)
