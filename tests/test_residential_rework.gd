...
func run() -> void:
	...
	for i in range(1,21):
		...
		for mat in building._night_materials: check(mat.get_shader_parameter("emission_energy")==2 and mat is ShaderMaterial,"Night emission missing")
		building.apply_night(0)
		for mat in building._night_materials: check(mat.get_shader_parameter("emission_energy")==0,"Daytime windows lit")
	check(tanks==3,"Expected three water-tower models")
	var sample: Node=world.get_child(17)
	var stable: PackedByteArray=sample._night_materials[0].get_shader_parameter("room_data").get_image().get_data()
	library.set_city_seed(54321)
	check(stable!=sample._night_materials[0].get_shader_parameter("room_data").get_image().get_data(),"Residential seeds should differ")
	library.restore_data({"seed":12345})
	check(stable==sample._night_materials[0].get_shader_parameter("room_data").get_image().get_data(),"Residential load must restore exact patterns")
	...
	print("RESIDENTIAL_PASS: models=20 placements=",placements," water-models=",tanks," failures=",failures)
