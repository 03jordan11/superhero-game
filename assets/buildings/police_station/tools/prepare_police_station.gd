extends SceneTree
const FOLDER := "res://assets/buildings/police_station/"
...
func _initialize() -> void:
	...
	var hall := Node3D.new()
	hall.name = "PoliceStation"
	hall.set_script(load(FOLDER + "police_station.gd"))
	owned(hall, (load(FOLDER + "police_station.glb") as PackedScene).instantiate(), "Model")
	var body := StaticBody3D.new()
	owned(hall, body, "ExteriorCollision")
	var manifest: Dictionary = JSON.parse_string(FileAccess.get_file_as_string(FOLDER + "police_station_manifest.json"))
	...
	for entry: Dictionary in manifest.props:
		...
		prop.owner = hall
	save(hall, FOLDER + "police_station.tscn")
	hall.free()
	print("POLICE STATION_SCENE_READY")
	quit()
