...
func run() -> void:
	...
	check(bindings.label_for("secondary_power", "keyboard") == "Q" and bindings.label_for("secondary_power", "controller") == "RT", "Power Special retains Q and RT defaults")
	check(bindings.action_label("secondary_power") == "Power Special", "Special-action binding is named independently of the selected power")
	check(bindings.action_label("dodge_roll") == "Dodge Roll", "Dodge binding uses its plain name")
	check(bindings.bindings.keyboard.dodge_roll.code == KEY_CTRL and bindings.bindings.keyboard.flight_descend.code == KEY_CTRL, "Dodge and flight descent share Ctrl")
	check(bindings.label_for("dodge_roll", "controller") == "B" and bindings.label_for("flight_descend", "controller") == "B", "Dodge and flight descent share B")
	var version_three := ConfigFile.new()
	bindings.write_config(version_three)
	version_three.set_value("bindings", "version", 3)
	version_three.set_value("bindings_controller", "dodge_roll", {"kind": "button", "code": JOY_BUTTON_DPAD_RIGHT})
	bindings.load_config(version_three)
	check(bindings.label_for("dodge_roll", "controller") == "B", "Saved old dodge default migrates to B")
	version_three.set_value("bindings_controller", "dodge_roll", {"kind": "button", "code": JOY_BUTTON_DPAD_UP})
	bindings.load_config(version_three)
	check(bindings.label_for("dodge_roll", "controller") == "D-pad Up", "Custom dodge binding survives migration")
	bindings.load_config(ConfigFile.new())
	var shared := ConfigFile.new()
	bindings.write_config(shared)
	shared.set_value("bindings_keyboard", "jump", {"kind": "key", "code": KEY_K})
	bindings.load_config(shared)
	check(bindings.label_for("dodge_roll", "controller") == "B" and bindings.label_for("flight_descend", "controller") == "B", "Shared B survives saving and loading")
	check(bindings.label_for("jump", "keyboard") == "K" and bindings.bindings.keyboard.dodge_roll.code == KEY_CTRL and bindings.bindings.keyboard.flight_descend.code == KEY_CTRL, "Shared bindings survive load without resetting custom controls")
	bindings.rebind("jump", "keyboard", key(KEY_CTRL))
	check(bindings.bindings.keyboard.jump.code == KEY_CTRL and bindings.bindings.keyboard.dodge_roll.code == KEY_K and bindings.bindings.keyboard.flight_descend.code == KEY_K, "Rebinding another action swaps the shared group safely")
	bindings.rebind("flight_descend", "keyboard", key(KEY_CTRL))
	check(bindings.bindings.keyboard.jump.code == KEY_K and bindings.bindings.keyboard.dodge_roll.code == KEY_CTRL and bindings.bindings.keyboard.flight_descend.code == KEY_CTRL, "Swapping from a shared group preserves valid assignments")
	bindings.load_config(ConfigFile.new())
