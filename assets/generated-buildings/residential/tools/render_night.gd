...
func render_all() -> void:
	DirAccess.make_dir_recursive_absolute("res://artifacts/residential_batch/night")
	...
	var sheet = Image.create(1800,1920,false,Image.FORMAT_RGB8)
	for i in range(1,21):
		if current != null:
			current.free()
		...
		if i in [1,4,8,10,14,19]:
			img.save_png("res://artifacts/residential_batch/night/building_%02d.png" % i)
		if i == 10:
			camera.position = Vector3(0,18,-50)
			camera.look_at(Vector3(0,16,0))
			await process_frame
			await process_frame
			await RenderingServer.frame_post_draw
			viewport.get_texture().get_image().save_png("res://artifacts/residential_batch/night/balcony_detail.png")
	sheet.save_png("res://artifacts/residential_batch/night/contact_sheet.png")
	...
	await process_frame
	await RenderingServer.frame_post_draw
	viewport.get_texture().get_image().save_png("res://artifacts/residential_batch/night/street_detail.png")
