extends SceneTree
const OUT := "res://assets/bridges/city_hall_arch/"
func _initialize() -> void: run.call_deferred()
func run() -> void:
	var source: Node3D = load(OUT + "city_hall_arch.glb").instantiate()
	var bridge := Node3D.new()
	bridge.name = "CityHallBridge"
	...
	road.set_shader_parameter("width_m", 28.0)
	road.set_shader_parameter("length_m", 320.0)
	...
	for original in source.find_children("*", "MeshInstance3D", true, false):
		...
		if original.name not in [&"Hangers"]:
			...
			if original.name in [&"RoadDeck", &"Walkways"]:
				...
				var strips: Array = [[-14.0, 14.0, 0.0]] if original.name == &"RoadDeck" else [[-18.0, -14.0, .15], [14.0, 18.0, .15]]
				for strip in strips:
	...
	bridge.set_meta("blender_source", OUT + "city_hall_arch.blend")
	for entry in [["WestRoadJoin", Vector3(0, .03, -160)], ["EastRoadJoin", Vector3(0, .03, 160)]]:
	...
	assert(ResourceSaver.save(packed, OUT + "city_hall_bridge.tscn") == OK)
