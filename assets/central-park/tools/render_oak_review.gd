...
func run() -> void:
	...
	for entry in [["ORIGINAL OAK",-9.0,"res://artifacts/central_park/oak_before_100.res"],
		["SIMPLIFIED OAK",9.0,"res://assets/central-park/meshes/tree_oak.res"]]:
		...
		label.font_size = 52
		label.pixel_size = .01
		label.position = Vector3(entry[1],15.2,0)
	...
	assert(viewport.get_texture().get_image().save_png("res://artifacts/central_park/oak_comparison.png") == OK)
	print("Captured oak comparison")
	viewport.free()
	quit()
