...
func _run_test() -> void:
	...
	assert(first.hud.has("ChargeUI") and first.hud.has("PerformanceHUD"))
	...
	assert(first.cpu_timings.hud_on_health_changed.calls > 0)
	...
	player.player_hud._on_health_changed(1.0, 2.0)
	assert(monitor.collect_sample().cpu_timings.is_empty())
	monitor.enabled = true
	player.player_hud._on_health_changed(1.0, 2.0)
	assert(monitor.collect_sample().cpu_timings.hud_on_health_changed.calls == 1)
	main.free()
	assert(MONITOR.begin() == 0)
